...
**5. `online` is retired as a Category; delivery channel becomes a facet on the Service it duplicates.** Its 5 Services (`online-business-billing`, `online-new-business`, `online-business-renewal`, `online-rpt-billing`, `online-payment-order`) are duplicate online-delivery records of Services already living under Business/Taxation — modeling delivery channel as a Category duplicated the same need twice. (An earlier draft of this ADR incorrectly attributed the duplication to a specific third-party e-payment platform Las Piñas does not use; that attribution is unfounded and has been dropped.) Reconciling each `online-*` record into its real counterpart is data work, filed as a follow-on ticket per this ticket's own AC — this ADR records only the call.
...
- `government` Category is removed from `categories.json`. No migration — it held no Services. `navigation.json`'s top-level `government` mainNav item (`/government`, the Office/Agency directory page) is unrelated — decision #3 already retired navigation.json's Category-hidden flags/entries (#284/#287, PR #289), so nothing Category-shaped remained there to remove; the id collision with the retired Category is coincidental.
