...
2. **`sources` present, no `verifiedOn`.** The citation list renders, followed by "Matched to this document after the fact. Not yet confirmed against the original." — this state specifically means the citation was matched to the record after the fact, not recorded by whoever transcribed it (pluralised "these documents" for more than one source).
