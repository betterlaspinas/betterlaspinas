...
- `pnp-laspinas` location/contact must be sourced; mark unverified until confirmed (ties to the deferred `dataStatus` flag). **Superseded by ADR-0005**: the Agency-scoped `dataStatus` field is deleted, and `pnp-laspinas` now carries `sources[]` — its `phone` and `location` are each verified, dated `2026-08-03`, with `covers` scoping which source backs which field.
