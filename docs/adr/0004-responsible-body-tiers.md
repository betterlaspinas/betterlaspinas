...
- `pnp-laspinas` location/contact must be sourced; mark unverified until confirmed (ties to the deferred `dataStatus` flag). **Superseded by ADR-0005**: the Agency-scoped `dataStatus` field is deleted, and `pnp-laspinas` now carries `sources[]` — it reads as unverified until a `verifiedOn` is stamped.
