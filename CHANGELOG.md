...
### Fixed
- Removed six Office phone numbers that were all the same `(078) 326-5001` placeholder — a Cagayan Valley area code, not Las Piñas — from `city-treasurer`, `city-assessor`, `city-agriculture`, `city-budget`, `city-accounting`, and `city-general-services` (#199). The department records these Offices duplicated held `(02)` numbers instead, but those are unsourced and the Citizen's Charter has no number for any of these offices, so the field is left blank rather than publishing an unverified contact number; sourcing them is tracked separately. Offices whose number is not the placeholder are untouched
