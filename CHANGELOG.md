...
- Retired the `online` Service Category (#288, per #255's ruling, ADR-0006): it modeled a delivery channel (Filipizen), not a task a resident came to do. 4 of its 5 Services were Filipizen-delivered duplicates of a Service already cataloged under Business/Taxation — `online-business-billing`/`online-rpt-billing` duplicated `business-tax`/`real-property-tax` respectively (View & Pay bills), `online-new-business` duplicated `business-permit-new`, `online-business-renewal` duplicated `business-permit-renewal` — so each was deleted and its Filipizen link became a delivery-channel fact on the real record instead: a new top-level `ServiceItem.onlineUrl` for the two catalog-only Taxation records (no `detail` page to carry `ServiceDetail.onlineLink`), and the two Business records already carried `detail.onlineLink` from a prior PR, so no new field was needed there. `online-payment-order` ("various city fees") had no confident 1:1 match — Filipizen's own `po/billing` module is a distinct miscellaneous-fees flow, not a duplicate of RPT or Business Tax billing — so it was recategorized to `tax-payments` as its own standalone Service instead of force-merged into one, pending a human call on whether it deserves closer folding later. `categories.json`'s `online` entry is deleted; `navigation.json` never referenced it (confirmed, no-op there, same as #286/#287)
