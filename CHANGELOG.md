...
- The Government page's "Department Heads & Key Offices" section now renders Offices joined to their heads through the `configHelper` accessor layer, grouped by Office Group, and prints the head's name for the first time; it replaces the dead placeholder grid that read `officials.departments` directly and linked the retired `/service-details/<slug>` namespace (#199, also removes one of the two dead blocks listed in #234)
...
### Fixed
- Corrected eight Office phone numbers that had drifted from the department records they duplicated (#199): six offices carried the same `(078) 326-5001` placeholder — an Isabela area code, not Las Piñas — and two carried none; all eight now use the `(02)` number recorded for that department. `city-engineering` and `cswdo` keep their existing Office numbers, which conflict with the department copy and need a source check
- Added a missing `Office` record for the Business Permits & Licensing Office (`business-permits-licensing`), which had a department entry but no canonical Office (#199, ADR-0003)
